...
class CnnBaseModel(AbstractModel):
    ...
    def create_validation_batch(self, batch_size=100, four_dim=False):
        while True:
            ...
            for idx in range(batch_size):
                img_num = np.random.choice(self.data_set_validation.shape[0])
                data_patch, veri_patch = utility.get_random_image_patch(self.data_set_validation[img_num],
                                                                        self.verifier_set_validation[img_num],
                                                                        self.patch_size,
                                                                        self.patch_size,
                                                                        self.context_padding)
                label = (np.mean(veri_patch) > 0.25) * 1
